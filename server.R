...
OnlineDataSort<-function(){
  ...
  data<-curl(urlfetch, open="r")
  urlcontent<-readLines(data)
  close(data)
  ...
  print(urlfetch2)
  data2<-curl(urlfetch2, open="r")
  urlcontent_SC<-readLines(data2)
  close(data2)
  urlcontent_SC <- gsub('<tr />', '', urlcontent_SC)
  SCDATA<- read.table(textConnection(urlcontent_SC), header=T, sep = '\t')
  head(SCDATA)
  print("b")
...
SatteliteDataSort<-function(SiteSpecificUrl){
  ...
  #Now we have both Discharge and SC data from the Sattelite and NWIS now merge and pad
  SCDf<-pad(SCDf, interval='15 min',start_val=datestring1, end_val = datestring2)
...
# Gardner R.  
  ...
  Data<-SatteliteDataSort('https://www.isi-data.com/Detail.aspx?lid=2015915727879&gid=1&')
  Discharge<-Data[,2]
  SC<-Data[,3]
  TimeDis<-Data[,1]
  TimeSC<-Data[,1]
